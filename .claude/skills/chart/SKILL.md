...
## Four Pillars 인포그래픽 내재화

사용자 요청에 `내재화해줘`, `인포그래픽으로 만들어줘`, `템플릿에 넣어줘`, 또는 `--infographic`이 포함되면 차트 생성 후 종료하지 않는다.

1. 사용자가 요청에 넣은 title과 subtitle을 그대로 유지한다.
   - 인포그래픽의 메인 title은 모든 단어의 첫 글자를 대문자로 변환한다. 나머지 글자는 원래 대소문자를 유지한다.
   - 인포그래픽의 subtitle에도 같은 대문자 변환 규칙을 적용한다.
   - 인포그래픽의 legend 표시명에도 같은 대문자 변환 규칙을 적용한다.
   - title이 여러 줄이면 문법적 의미 단위를 우선해 자연스럽게 나누고, 그 안에서 줄 길이를 균형 있게 맞춘다. 전치사는 뒤의 목적어와 같은 줄에 두어 전치사구를 보존하고, 관사와 접속사도 줄 끝에 홀로 남기지 않는다. 월과 연도, 수치와 단위, 고유명사를 갈라놓거나 마지막 줄에 한 단어만 남기지 않는다.
2. 차트에 실제로 사용한 Source와 데이터의 마지막 날짜를 확정한다. 추측하지 않으며 `Date as of`에는 종료일만 전달한다.
3. Four Pillars 투명 배경 SVG를 생성한다.
4. `.claude/skills/assemble-infographic/SKILL.md`를 읽고 최종 1920x1080 인포그래픽 SVG와 PNG를 조립한다.
5. 렌더링된 PNG를 열어 겹침, 잘림, 정렬을 검사한 뒤 통과한 결과만 반환한다.

Note는 사용자 요청에 있을 때만 전달한다. 없으면 만들지 않는다.
...
- **범례(legend) 넣지 않음** - 차트 이미지에 범례를 포함하지 않는다
  - 인포그래픽 내재화 요청에서는 시리즈명, 실제 색상, 선 스타일을 조립 단계로 전달한다. `assemble-infographic` 스킬이 차트 외부 범례를 만든다
