"""HYPE vs BTC 지수 차트, Hyperliquid API 직접 호출 버전.

로컬 파일 버전은 charts/hype_vs_btc_indexed_2q26.py.
HYPE는 @107(HYPE/USDC 스팟), BTC는 perp. 둘 다 1d 캔들 종가.
"""
import pandas as pd
...
def daily_closes(coin, start, end):
    """Hyperliquid 1d 캔들 종가. coin은 perp이면 'BTC', 스팟이면 '@107' 형식"""
    ...
    d = pd.DataFrame(json.load(urllib.request.urlopen(req, timeout=30)))
    d['date'] = pd.to_datetime(d['t'], unit='ms')
    return d.set_index('date')['c'].astype(float)


hype = daily_closes('@107', '2026-03-30', '2026-07-01')  # HYPE/USDC 스팟
btc = daily_closes('BTC', '2026-03-30', '2026-07-01')    # BTC perp
...
name = 'hype_vs_btc_indexed_2q26_api'
...
plt.close(fig)
print(f'HYPE {hype_i.iloc[-1] - 100:+.1f}%  BTC {btc_i.iloc[-1] - 100:+.1f}%')
