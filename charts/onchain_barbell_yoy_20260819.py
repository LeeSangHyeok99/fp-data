...
df = pd.read_csv("outputs/data/onchain_barbell_yoy_20260819.csv")
...
# 밴드 폭은 담는 라벨 밀도에 비례. Onchain Native가 10개로 가장 빽빽하다.
WIDTHS = {"Speculative": 8.0, "Onchain Native": 13.0, "TradFi-linked": 9.0}
EDGES = np.cumsum([0.0] + [WIDTHS[g] for g in GROUPS])
TOTAL = EDGES[-1]
PAD = 0.6  # 밴드 좌우 안쪽 여백 (라벨이 구분선을 넘지 않게)

# 성장 배수의 로그. -100%면 0배라 정의역 밖이므로 데이터에 없다고 전제.
y = np.log10(1 + df["pct"] / 100)
x = df["group"].map(lambda g: EDGES[GROUPS.index(g)] + PAD) + df["x"] * df["group"].map(
    lambda g: WIDTHS[g] - 2 * PAD
)
...
ax.set_xlim(0, TOTAL)
ax.set_ylim(np.log10(0.115), np.log10(70))
...
ax.set_xticks([(EDGES[i] + EDGES[i + 1]) / 2 for i in range(len(GROUPS))])
...
for i in range(1, len(GROUPS)):
    ax.axvline(EDGES[i], color=COLORS["grid"], alpha=0.45, lw=1.2,
...
fig.tight_layout()
png, svg = save_chart(fig, "onchain_barbell_yoy_20260819", "outputs/charts/market/structure")
print(png)
print(svg)
