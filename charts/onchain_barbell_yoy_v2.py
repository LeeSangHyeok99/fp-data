"""Onchain barbell v2 — YoY change in key metrics, grouped dot plot.
3 buckets (Speculative, Onchain Native, TradFi-linked), 19 metrics.

기준일 D = 2026-08-20 (전 지표 공통, 각 지표 최신 가용일 중 가장 이른 날).
stock 지표는 D 시점 값, flow 지표(거래량/매출)는 D까지 30일 합을 1년 전 같은
창과 비교한다. 데이터 수집은 scripts/fetch_onchain_barbell_yoy.py.
...
df = pd.read_csv("outputs/data/onchain_barbell_yoy_v2.csv")
...
ax.set_ylim(np.log10(0.18), np.log10(70))

# 5배 간격 틱 (로그 축에서 등간격)
ticks = [0.2, 1, 5, 25]
ax.yaxis.set_major_locator(FixedLocator([np.log10(t) for t in ticks]))
ax.yaxis.set_major_formatter(
    FuncFormatter(lambda v, _: f"{10 ** v:.1f}x" if 10 ** v < 1 else f"{10 ** v:.0f}x")
)
...
OFFSETS = {
    "right": ((11, 6), (11, -8), "left"),
    "above": ((0, 24), (0, 10), "center"),
    "below": ((0, -13), (0, -27), "center"),
}
...
fig.tight_layout()
png, svg = save_chart(fig, "onchain_barbell_yoy_v2", "outputs/charts/market/structure")
print(png)
print(svg)
