...
SRC = 'sources/hyperliquid-q2-data/asxn/raw/HL_BUYBACKS.json'
df = pd.DataFrame(json.load(open(SRC)))
df['date'] = pd.to_datetime(df['date'])

# 일별 매입 금액(ntl)을 주간 합산. 마지막 완전 주 2026-W26(6/28)까지
df = df[df['date'] <= '2026-06-28']
wk = (df.set_index('date')['ntl'].resample('W-SUN').sum() / 1e6).reset_index()
wk.columns = ['week_end', 'buyback_musd']
wk = wk[wk['week_end'] <= '2026-06-28'].reset_index(drop=True)
...
# 2026Q2 강조 (주 종료일 2026-04-05 ~ 2026-06-28)
...
x = range(len(wk))
ax.bar(x, wk['buyback_musd'], width=0.8, color=colors, zorder=2)

q2_idx = [i for i, q in enumerate(is_q2) if q]
ax.text(sum(q2_idx) / len(q2_idx), 36, '2026 Q2', ha='center', va='bottom',
...
tick_pos = list(range(3, len(wk), 13))
...
name = 'asxn_af_weekly_buyback_since_inception'
