...
# Data
# =============================================================================
df = pd.read_csv('sources/figure10_neverland_tvl_borrows_fees.csv',
                 parse_dates=['date_utc']).rename(columns={'date_utc': 'date'})

COLOR_TVL = '#2C3AC4'      # 네이비 (원본 #192170에서 밝기만 상향)
COLOR_BORROWS = '#89009C'  # 플럼 (원본 #480052에서 밝기만 상향)
COLOR_FEES = '#B506F5'     # 브라이트 마젠타
...
# Neverland TVL (피크 $45.5M)
line_area_chart(df['tvl_excluding_borrowed_usd'] / 1e6, COLOR_TVL,
                [0, 10, 20, 30, 40], ['$0M', '$10M', '$20M', '$30M', '$40M'],
                47, 'outputs/charts/neverland/tvl', 'neverland_tvl_tall')

# Neverland 차입금 (피크 $122.5M)
line_area_chart(df['borrowed_usd'] / 1e6, COLOR_BORROWS,
                [0, 40, 80, 120], ['$0M', '$40M', '$80M', '$120M'],
                128, 'outputs/charts/neverland/borrows', 'neverland_borrows_tall')

# Neverland 일일 수수료 (피크 $29.2K)
bar_chart(df['fees_usd'] / 1e3, COLOR_FEES,
          [0, 10, 20, 30], ['$0K', '$10K', '$20K', '$30K'],
          30.5, 'outputs/charts/neverland/fees', 'neverland_daily_fees_tall')
